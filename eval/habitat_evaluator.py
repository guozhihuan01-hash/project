...
class HabitatEvaluator:
    ...
    def load_scene(self, scene_id: str):
        ...
        # 把 episode 里的 hm3d_v0.2 路径改成你本地实际的 hm3d 路径
        scene_id = scene_id.replace("hm3d_v0.2/", "hm3d/")
        scene_id = scene_id.lstrip("/")

        backend_cfg = habitat_sim.SimulatorConfiguration()

        if self.is_gibson:
            backend_cfg.scene_id = os.path.join(self.scene_path, scene_id)
        else:
            # 根据 scene_id 判断 split
            split = "val" if "/val/" in scene_id else "train"
        ...
        print("scene_id =", backend_cfg.scene_id)
        if not self.is_gibson:
            print("scene_dataset_config_file =", backend_cfg.scene_dataset_config_file)
        ...
        if self.scene_data[original_scene_id].objects_loaded:
            return

        if not self.is_gibson:
            self.scene_data = HM3DDataset.load_hm3d_objects(
                self.scene_data, self.sim.semantic_scene.objects, original_scene_id
        )
        else:
            self.scene_data = GibsonDataset.load_gibson_objects(
    ...
    def evaluate(self):
        ...
        results = []
        # restart at 930
        for n_ep, episode in enumerate(self.episodes[:5]):
