...
from mapping.adaptive_navigator import AdaptiveNavigator
from planning import Planning, Controllers
...
class MONActor(Actor):
    def __init__(self, config):
        if getattr(config, "log_rerun", False):
            try:
                rr.init("onemap_apexnav", spawn=True)
                rr.log("debug/start", rr.TextLog("rerun connected"))
                print("Rerun connected")
            except Exception as e:
                print(f"Rerun init failed: {e}")
        model = ClipModel("weights/clip.pth", jetson=False)
        detector = YOLOWorldDetector(config.planner.yolo_confidence) if config.planner.using_ov \
            else YOLOv7Detector(config.planner.yolo_confidence)
        ...
        self.mapper = AdaptiveNavigator(model, detector, config)
    ...
    def act(self, observations: Dict[str, any]) -> Tuple[Dict, bool]:
        ...
        r = R.from_euler("xyz", [0, 0, yaw]).as_matrix()
        ...
        if path is not None and len(path) > 0:
            ...
            rr.log(
                ...
                rr.LineStrips2D(
                    path,
                    colors=np.repeat(np.array([0, 0, 255])[np.newaxis, :], path.shape[0], axis=0)
                )
            )
            ...
            ang, lin = self.controller.control(pos, yaw, path, False)
            return_act['continuous'] = {'linear': lin, 'angular': ang}
            return return_act, obj_found
